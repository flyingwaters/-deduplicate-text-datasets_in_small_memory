'''
Author: Fenglongyu
Date: 2022-05-05 23:08:01
LastEditTime: 2022-05-22 02:43:35
FilePath: /project/deduplicate-text-datasets/scripts/make_suffix_array.py
Description: 

Copyright (c) 2022 by fenglongyu, All Rights Reserved. 
'''
...
for jobstart in range(0, total_jobs, jobs_at_once):
    ...
    for i in range(jobstart,jobstart+jobs_at_once):
        ...
        cmd = "%s/target/debug/dedup_dataset make-part --data-file %s --start-byte %d --end-byte %d"%(project_path,sys.argv[1], s, e)
        started.append((s, e))
        print(cmd)
...
os.popen("rm -rf %s/tmp/out.table.bin.*"%(project_path)).read()

torun = " --suffix-path ".join(files)
print("%s/target/debug/dedup_dataset merge --output-file %s --suffix-path %s --num-threads %d"%(project_path,"/raid/zyftest/project/deduplicate-text-datasets/tmp/out.table.bin", torun, mp.cpu_count()))
pipe = os.popen("%s/target/debug/dedup_dataset merge --output-file %s --suffix-path %s --num-threads %d"%(project_path, "/raid/zyftest/project/deduplicate-text-datasets/tmp/out.table.bin", torun, mp.cpu_count()))
output = pipe.read()
...
os.popen("cat /raid/zyftest/project/deduplicate-text-datasets/tmp/out.table.bin.* > /raid/zyftest/project/deduplicate-text-datasets/tmp/out.table.bin").read()
print("Cleaning up")
os.popen("mv /raid/zyftest/project/deduplicate-text-datasets/tmp/out.table.bin %s.table.bin"%sys.argv[1]).read()
